...
int main()
{
    ...
    timestamp();
    printf("\n");
    printf("Program Uses the OpenMP API to parallelize the Jacobi Iterative Solver to decrease computation time to solve the 2D Poisson's Equation.\n");
    printf("\n");
...
// #pragma omp parallel
    ...
    */
    ...
    for (;;)
    {
        ...
        printf("  %4d  %14g\n", output_iteration, potential_new_norm);
...
// #pragma omp parallel \
shared(dx, dy, f, output_iteration, input_iteration, nx, ny, potential_array, potential_new_array) private(i, iter, j)

    for (iter = input_iteration + 1; iter <= output_iteration; iter++)
    {
